...
export default function GstCalculatorPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 px-4 py-8 md:py-12">
      <Helmet>
        <title>GST Calculator: Online Indian GST Calculator | ServiceLocal</title>
        <meta
          name="description"
          content="Instantly calculate GST in India. Add GST to a price or remove GST from an inclusive amount using our free online GST calculator."
        />
      </Helmet>
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
              ...
              <p className="mt-1 text-lg font-bold text-slate-900">
                {result.hasValue ? formatINR(result.taxableAmount) : '—'}
              </p>
            </div>
            <div className="rounded-xl bg-emerald-50 border border-emerald-100 p-4">
              <p className="text-sm text-slate-500">GST Amount ({gstRate}%)</p>
              <p className="mt-1 text-lg font-bold text-emerald-700">
                {result.hasValue ? formatINR(result.gstAmount) : '—'}
              </p>
            </div>
            <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
              <p className="text-sm text-slate-500">Total Amount</p>
              <p className="mt-1 text-lg font-bold text-slate-900">
                {result.hasValue ? formatINR(result.totalAmount) : '—'}
              </p>
            </div>
          </div>
